---
title: "Introduction au traitement des données géospatiales avec R"
...
```{r read_mrc, warning = FALSE, message = FALSE}
library(sf)
...
```{r sf_dplyr, message = FALSE, warning = FALSE}
library(dplyr)
...
Le fichier `plots_rgeo.csv` contient des données sur les placettes d'inventaire du Ministère des Forêts, de la Faune et des Parcs du Québec (MFFP), incluant le numéro d'identification, la latitude et la longitude, la date d'inventaire, le type de couvert (feuillu, mixte ou confière) ainsi que la classe de hauteur. 
...
```{r ggplot_bar, message = FALSE, warning = FALSE}
library(ggplot2)
...
* L'argument `data` spécifie un jeu de données et la fonction `aes` associe les variables de ce jeu de données à des éléments graphiques. Ces arguments peuvent être définis dans la fonction `ggplot` (s'ils s'appliquent à toutes les couches) ou dans des couches `geom` spécifiques.
...
a) Combien de placettes dans ces régions sont affectées à chaque niveau de défoliation? *Indice*: La fonction `table` est utile pour compter les effectifs de chaque valeur dans une colonne. 
...
Ensuite, nous verrons trois fonctions basées sur des opérations d'ensembles. Si *A* et *B* sont des éléments géométriques, leur union est la zone couverte par A ou B, leur intersection est la zone couverte par A et B, et la différence (A - B) est la zone couverte par A, mais pas B. Dans `sf`, ces opérations sont réalisées par` st_union`, `st_intersection` et` st_difference`. 
...
```{r read_stars, message = FALSE, warning = FALSE}
library(stars)
...
a) Affichez une carte montrant à quels endroits l'élévation est entre 5 et 100 m dans la MRC de La Mitis.
...
```{r extract_pt, message = FALSE, warning = FALSE}
library(raster)
...
* Le manuel en-ligne [Spatial Data Science](https://keen-swartz-3146c4.netlify.com/) d'Edzer Pebesma et Roger Bivand présente les packages ***sf*** et ***stars*** en plus de détails, en plus d'inclure des chapitres sur l'analyse statistique de donnnées spatiales.

* [Geocomputation with R](https://geocompr.robinlovelace.net/) (Lovelace, Nowosad et Muenchow) est un autre manuel complet et gratuit sur le traitement et l'analyse de données spatiales.
...
* Le site web [RSpatial](https://rspatial.org/) (à ne pas confondre avec le précédent) inclut des tutoriels pour les packages *raster* et *terra* pour travailler avec des données matricielles.
